# SharePoint & OneDrive Sharing Audit — Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Build a PowerShell script that audits all sharing permissions across SharePoint and OneDrive in a Microsoft 365 tenant, outputting a CSV report.

**Architecture:** Single PowerShell script using Microsoft Graph PowerShell SDK with interactive auth. Recursively walks OneDrive and SharePoint document libraries, collects sharing permissions per item, exports to CSV.

**Tech Stack:** PowerShell 7+, Microsoft.Graph PowerShell SDK (Microsoft.Graph.Users, Microsoft.Graph.Sites, Microsoft.Graph.Files)

---

### Task 1: Script Scaffold — Parameters, Prerequisites, and Auth

**Files:**
- Create: `Invoke-SharingAudit.ps1`

**Step 1: Create the script with param block and prerequisites check**

Write the following to `Invoke-SharingAudit.ps1`:
...
Run: `pwsh -NoExecute -File Invoke-SharingAudit.ps1`
Expected: No parse errors (the script won't execute, just syntax-check)

**Step 3: Commit**

```bash
git add Invoke-SharingAudit.ps1
git commit -m "feat: add script scaffold with params, prereqs check, and auth"
```

---

### Task 2: Helper Functions — Retry, Classify, Record

**Files:**
- Modify: `Invoke-SharingAudit.ps1` (append after the results collection section)

**Step 1: Add the Invoke-WithRetry helper**

Append this after the `$script:totalSharedItems = 0` line:
...
```bash
git add Invoke-SharingAudit.ps1
git commit -m "feat: add helper functions for retry, classification, and recording"
```

---

### Task 3: Recursive Drive Walker

**Files:**
- Modify: `Invoke-SharingAudit.ps1` (append after the helper functions)

**Step 1: Add the Get-DriveItemsRecursive function and permission processor**
...
```bash
git add Invoke-SharingAudit.ps1
git commit -m "feat: add recursive drive walker with permission collection"
```

---

### Task 4: OneDrive Audit Logic

**Files:**
- Modify: `Invoke-SharingAudit.ps1` (append after the drive walker)

**Step 1: Add the OneDrive audit section**
...
```bash
git add Invoke-SharingAudit.ps1
git commit -m "feat: add OneDrive audit logic with user enumeration and progress"
```

---

### Task 5: SharePoint Audit Logic

**Files:**
- Modify: `Invoke-SharingAudit.ps1` (append after the OneDrive section)

**Step 1: Add the SharePoint audit section**
...
```bash
git add Invoke-SharingAudit.ps1
git commit -m "feat: add SharePoint site and document library audit logic"
```

---

### Task 6: CSV Export and Console Summary

**Files:**
- Modify: `Invoke-SharingAudit.ps1` (append after the SharePoint section)

**Step 1: Add CSV export and summary**
...
**Step 2: Verify the complete script parses without error**
...
```bash
git add Invoke-SharingAudit.ps1
git commit -m "feat: add CSV export and console summary with risk highlights"
```
...
**Files:**
- Read: `Invoke-SharingAudit.ps1` (full review)

**Step 1: Review the complete script end-to-end**

Read the entire file and verify:
- [ ] Parameters are correctly defined
- [ ] All helper functions are called with correct argument names
- [ ] Recursive walker correctly passes through all parameters
- [ ] CSV columns match the design doc
- [ ] Progress bars use distinct `-Id` values
- [ ] No variable name mismatches between sections
- [ ] `$script:results` is used consistently
...
```bash
git add Invoke-SharingAudit.ps1
git commit -m "chore: finalize script after end-to-end review"
```
...
After all tasks complete, the user should:

1. Install the Graph modules: `Install-Module Microsoft.Graph -Scope CurrentUser`
2. Run: `./Invoke-SharingAudit.ps1`
3. Sign in with a Global Admin or SharePoint Admin account
4. Wait for completion (estimate: 15-45 min for medium tenant)
5. Open the CSV in Excel and filter by `SharingType` or `SharedWithType` to prioritize cleanup
